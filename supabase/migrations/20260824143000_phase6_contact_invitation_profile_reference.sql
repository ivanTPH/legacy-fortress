-- The hosted Phase 6 invitation RPC must use the canonical profile table.
-- The legacy profile relation is not part of the deployed schema; user_profiles is.
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(NULLIF(trim(up.display_name), ''), split_part(COALESCE(owner_user.email, ''), '@', 1), 'Account holder') AS account_holder_name,
    c.relationship
