...
export default function PlatformOrganisationControlCentre({ organisationId }: { organisationId?: string }) {
  ...
  function renderDetail() {
    ...
    return <div style={stackStyle}>
      ...
      <section style={actionRowStyle}><button type="button" style={primaryButtonStyle} disabled={!canManage} onClick={() => { setForm(toForm(org)); setEditOpen(true); }}>Edit organisation</button>{licence ? <Link href={`/admin/licences/${licence.id}`} style={secondaryLinkStyle}>View licence</Link> : <button type="button" style={secondaryButtonStyle} disabled={!canCreateLicence} onClick={() => setLicenceOpen(true)}>Allocate licence</button>}<button type="button" style={secondaryButtonStyle} disabled={!canInvite || ["suspended", "cancelled", "archived"].includes(org.status)} onClick={() => setInviteOpen(true)}>Invite administrator</button>{canSuspend || canReactivate ? <button type="button" style={dangerButtonStyle} onClick={() => setConfirmState(canReactivate ? "reactivate" : "suspend")}>{canReactivate ? "Reactivate" : "Suspend"}</button> : null}</section>
