...
export default function AccessRequestsWorkspace() {
  ...
  async function submitEvidence(file: File) {
    ...
    if (viewer.readOnly) {
      setStatus("❌ Linked access is view-only. Access evidence cannot be submitted from this account.");
      return;
    }
    ...
    setStatus("✅ Access request submitted for review.");
    ...
    <section style={{ display: "grid", gap: 14 }}>
      <div style={{ display: "grid", gap: 6 }}>
        ...
        <p style={{ margin: 0, color: "#6b7280" }}>
          Handle applicant details, updated contact information, death certificate evidence, verification checks, and manual review for access decisions in one workflow.
        </p>
      </div>
      ...
      <section style={workflowGridStyle}>
        {[
          ["1. Applicant details", "Choose the executor or power-of-attorney assignment that needs access."],
          ["2. Updated contact details", "Keep the linked person record current in Contacts before sending or reviewing access."],
          ["3. Death certificate upload", "Upload the supporting certificate or evidence file here."],
          ["4. Two-factor verification", "Security settings remain the place for mobile verification and secure notification controls."],
          ["5. Manual admin review", "Submitted evidence is reviewed through the admin verification queue before access activates."],
          ["6. Access decision", "Once approved, the linked contact sees only the records and permissions assigned to them."],
      ...
      <section style={cardStyle}>
        ...
        {!loading && viewer.readOnly ? (
          <div style={{ color: "#64748b", fontSize: 13 }}>
            This linked account is view-only. You can review request status here, but only the account owner can submit or replace evidence from this workspace today.
          </div>
        ) : null}
        {!loading && assignments.length === 0 ? (
          <div style={{ color: "#64748b", fontSize: 13 }}>
            No executor or power-of-attorney role assignments found. Add invitees from Contacts first.
          </div>
        ) : null}
      ...
      <section style={cardStyle}>
        ...
        {requests.length === 0 ? <div style={{ color: "#64748b" }}>No access requests submitted yet.</div> : null}
